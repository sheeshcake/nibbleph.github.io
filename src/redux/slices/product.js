...
const slice = createSlice({
  ...
  reducers: {
    ...
    updateCart(state, action) {
      const newProduct = action.payload;
      const isEmptyCart = !state.checkout.cart.length;
      console.log(newProduct, isEmptyCart)
      if (isEmptyCart) {
        state.checkout.cart = [...state.checkout.cart, newProduct];
      } else {
        state.checkout.cart = state.checkout.cart.map((product) => {
      ...
      console.log('State SHIT', state.checkout.cart.map((product) => {
        const isExisted = product.id === newProduct.id;
        if (isExisted) {
          return {
            ...newProduct
    ...
    removeItem(state, action) {
      ...
      state.checkout.cart = filter(state.checkout.cart, (obj) => !includes(ids, obj.id));
    },
